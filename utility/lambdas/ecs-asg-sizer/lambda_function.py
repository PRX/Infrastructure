...
def lambda_handler(event, context):
    ...
    usages = list(
        filter(
            lambda u: u["disconnectseconds"] < DISCONNECT_THRESHOLD_SECONDS, all_usages
        )
    )
...
def get_max_reservations(cluster_name):
    ...
    page_iterator = paginator.paginate(
        cluster=cluster_name, PaginationConfig={"PageSize": 10}
    )
...
def scale_asg(asg_name, delta, dry_run):
    group = asg_client.describe_auto_scaling_groups(AutoScalingGroupNames=[asg_name])[
        "AutoScalingGroups"
    ][0]
    asg_data = {
        "min": group["MinSize"],
    ...
    is_ok = asg_data["next"] >= asg_data["min"] and asg_data["next"] <= asg_data["max"]
