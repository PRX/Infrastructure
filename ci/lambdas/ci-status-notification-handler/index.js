...
function attachmentsForGitHubEvent(gitHubEvent, gitHubBuild) {
  ...
  const branch = gitHubEvent.pull_request
  ...
  if (gitHubEvent.pull_request) {
    ...
    const action =
      gitHubEvent.action.charAt(0).toUpperCase() + gitHubEvent.action.slice(1);
    ...
    const text = [];
    text.push(
      `<${compareUrl}|${gitHubEvent.commits.length} new commits> pushed by ${gitHubEvent.pusher.name}`,
    );
...
function attachmentsForCiCallback(ciResult) {
  ...
  if (ciResult.success) {
    ...
    if (ciResult.prxGithubPr) {
      ...
      const ecrUrl = `https://console.aws.amazon.com/ecs/home?region=${
        ciResult.prxEcrRegion
