...
The Git commit hash of the Infrastructure repo is used as a prefix in S3 for the copy. Another [custom Lambda](https://github.com/PRX/Infrastructure/blob/master/cd/lambdas/infrastructure-commit-proxy/lambda_function.py) action captures the commit hash in an artifact that can be passed around amongst CodePipeline actions. The hash value is not natively exposed by CodePipeline in such a way that it can be passed to the stack as a parameter. That proxy artifact is passed to the `deploy` action, and the Git hash value is extracted from the artifact and passed to the root stack as a parameter (using `ParameterOverrides`).
