...
- **Pay attention to property types**: There are many cases where a property value for a resource type will look like it takes one type, but actually takes another. Number values and booleans can be strings. It's very unusual for CloudFormation to not handle mismatches correctly, but do your best to have the template match what's expected and documented. `"1234"`, `"true"`, etc. But also the documentation is wrong some times.
