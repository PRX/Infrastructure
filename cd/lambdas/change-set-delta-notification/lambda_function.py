...
def slack_message(notification, deploy_id):
    return {
        ...
        "attachments": [parameters_delta_attachment(notification, deploy_id)],
    }
...
def deploy_id(artifacts):
    ...
    repo_artifact = next((a for a in artifacts if a["name"] == name), None)

    name = "TemplateConfigStagingZipArtifact"
    staging_config_artifact = next((a for a in artifacts if a["name"] == name), None)
